/* Test of QIO with data of type "real" array */
...
int main(int argc, char *argv[]){
  ...
  QDP_Real *field_in[NFIELD], *field_out[NFIELD];
  ...
  for(i = 0; i < NFIELD; i++)field_out[i] = QDP_create_R();
  printf("QDP fields created\n");

  /* Set some values */
  set_real(field_out[0]);
  printf("QDP field[0] values set\n");

  /* Build more */
  for(i = 1; i < NFIELD; i++)
    QDP_F_R_eq_r_times_R(field_out[i],&fact,field_out[i-1],QDP_all);
  ...
  status = QDP_F_vwrite_R(outfile, xml_record_out, field_out, NFIELD);
  printf("Wrote record with status %d\n",status);
  QDP_close_write(outfile);
  printf("Closed file\n");

  /* Set up a dummy input QDP field */
  for(i = 0; i < NFIELD; i++)field_in[i] = QDP_create_R();
  ...
  /* Read the record */
  status = QDP_F_vread_R(infile, xml_record_in, field_in, NFIELD);
  printf("QDP_F_read_vR returns status %d\n",status);
  QDP_close_read(infile);

  /* Compare the input and output fields */
  for(i = 0; i < NFIELD; i++){
    QDP_R_meq_R(field_out[i], field_in[i], QDP_all);
    QDP_r_eq_norm2_R(&diff, field_out[i], QDP_all);
    if(QDP_this_node == 0)
      printf("Comparision of %d in and out fields |in - out|^2 = %e\n",
	     i,diff);
  }

  /* free fields */
  for(i = 0; i < NFIELD; i++){
    QDP_destroy_R(field_in[i]);
    QDP_destroy_R(field_out[i]);
  }
