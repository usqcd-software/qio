/* Test of partitioned QIO with data of type "real" */
...
void fill_real(QLA_Real *r, int coords[]){
  ...
  *r = coords[3] + lattice_size[3]*(coords[2] + lattice_size[2]*
				   (coords[1] + lattice_size[1]*coords[0]));
}
...
int main(int argc, char *argv[]){
  ...
  printf("(%d)Record XML set\n",QDP_this_node);

  /* Write the field to a file */
  /* This is not the way we should do it -- 
     the implementation is supposed to make the PARTFILE choice! */
  outfile = QDP_open_write(xml_file_out, filename, QIO_PARTFILE, 0);
  printf("Opened file for writing\n");
  status = QDP_write_R(outfile, xml_record_out, field_out);
  printf("(%d)Wrote field with status %d\n",QDP_this_node,status);fflush(stdout);
  QDP_close_write(outfile);
  printf("(%d)closed file\n",QDP_this_node);fflush(stdout);
  ...
  printf("(%d)QDP_read_R returns status %d\n",QDP_this_node,status);
  QDP_close_read(infile);

  /* Compare the input and output fields */
  QDP_R_meq_R(field_out, field_in, QDP_all);
  printf("(%d)QDP_R_meq_R finished\n",QDP_this_node);
  QDP_r_eq_norm2_R(&diff, field_out, QDP_all);
  printf("(%d)QDP_r_eq_norm2_R finished\n",QDP_this_node);
  if(QDP_this_node == 0)
    printf("Comparison of in and out fields |in - out|^2 = %e\n",diff);
