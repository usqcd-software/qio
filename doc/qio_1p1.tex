...
\begin{document}
\title{
   Input/Output for QCD \\{\large Version 1.1}
}
...
members specify the lattice coordinate extent and spacetime
dimensionality.  The sixth member specifies the full spacetime volume.
The seventh, the number of sites on the current node, the eighth, the
number of the present node, and the ninth, the total number of nodes.
...
assumed that the user has prepared it as described above.  The
\verb|oflag| parameter is reserved for future enhancements.
...
described above.  The \verb|iflag| parameter is reserved for future
enhancements.
